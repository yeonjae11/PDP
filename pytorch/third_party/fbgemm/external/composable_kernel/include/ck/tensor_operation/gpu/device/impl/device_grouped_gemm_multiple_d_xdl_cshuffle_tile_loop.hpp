                                       ...
                                       CDEElementwiseOperation>
{
    ...
    struct Invoker : public BaseInvoker
    {
        ...
        static constexpr index_t MinimumOccupancy = []() {
            ...
            {
                constexpr int a_tile_size = MPerBlock * KPerBlock * sizeof(ADataType);
                constexpr int b_tile_size = NPerBlock * KPerBlock * sizeof(BDataType);
                constexpr int acc_tile_size = MPerBlock * NPerBlock * sizeof(AccDataType);

                constexpr int reg_size = (a_tile_size + b_tile_size + acc_tile_size) / BlockSize;
                constexpr int lds_size = (a_tile_size + b_tile_size);

                constexpr int occupancy_0 =  64 * 1024 / lds_size;
                constexpr int occupancy_1 =  256 * 4 / reg_size;

                return occupancy_0 or occupancy_1 < 2 ? 1 : 2;
            }
            else
            {
                return 1;
